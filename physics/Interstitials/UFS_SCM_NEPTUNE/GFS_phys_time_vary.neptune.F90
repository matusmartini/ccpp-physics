...
!> @{
      subroutine GFS_phys_time_vary_init (                                                         &
              mpicomm, mpirank, mpiroot, ntoz, h2o_phys, iaerclm, iccn, iaermdl, iflip, im, levs,  &
...
!>  added coupled gocart and radiation option to initializing aer_nm
         if (iaerclm) then
           ...
           call read_aerdata (mpicomm,mpirank,mpiroot,iflip,idate,myerrmsg,myerrflg)
           call copy_error(myerrmsg, myerrflg, errmsg, errflg)
...
!> - Call tau_amf dats for  ugwp_v1
         if (do_ugwp_v1) then
            ...
            call read_tau_amf(mpicomm, mpirank, mpiroot, myerrmsg, myerrflg)
            call copy_error(myerrmsg, myerrflg, errmsg, errflg)
         endif
...
!> - Initialize soil vegetation (needed for sncovr calculation further down)
         ...
         call set_soilveg(mpirank, isot, ivegsrc, nlunit, myerrmsg, myerrflg)
         call copy_error(myerrmsg, myerrflg, errmsg, errflg)
...
!> - Call gcycle() to repopulate specific time-varying surface properties for AMIP/forecast runs
         if (nscyc >  0) then
           ...
           !  call gcycle (mpirank, nthrds, nx, ny, isc, jsc, nsst, tile_num, nlunit,              &
